...
// ssid="GC-Automation";
 ...
 clientSecurity="";
// password="default";
strcpy(clientPassword,""); //Wifi Settings

// ssid="GC-Automation";
 strcpy(apssid,"GC-Automation");
 apSecurity="WPA";
// password="default";
strcpy(apPassword,"default");
...
void handleRoot()
{
  if (server.hasArg("wifiType")) {
    ...
    if (sWifiType="ap"){
      strcpy(apssid,server.arg("apssid").c_str());
      apSecurity=server.arg("apSecurity");
     strcpy(apPassword,server.arg("apPassword").c_str());
    }
    else if(sWifiType="client"){
      strcpy(clientssid,server.arg("clientssid").c_str());
      clientSecurity=server.arg("clientSecurity");
     strcpy(clientPassword,server.arg("clientPassword").c_str());
    }
    ...
    for(int i=1;i<5;i++){
      if (server.hasArg("channel"+String(i)+"-name")){
        channelNames[i-1]=server.arg("channel"+String(i)+"-name");
      }
      else{channelNames[i-1]="Channel"+i;
    }

    if (server.hasArg("channel"+String(i)+"-timer")){
      channelTimers[i-1]=String(server.arg("channel"+String(i)+"-timer")).toInt();
    }
    else{channelTimers[i-1]=5;
  }
  if (server.hasArg("channel"+String(i)+"-time-stamp")){
    channelNames[i-1]=server.arg("channel"+String(i)+"-time-stamp");
    if (server.arg("channel"+String(i)+"-time-stamp")=="min") {
      channelTimers[i-1]=channelTimers[i-1]*60;
    }
    ...
    INDEX_HTML += channelNames[0];
    INDEX_HTML +="\"); $(\"#chn-2-name\").text(\"";
    INDEX_HTML +=  channelNames[1];
    INDEX_HTML +="\"); $(\"#chn-3-name\").text(\"";
    INDEX_HTML += channelNames[2];
    INDEX_HTML +="\"); $(\"#chn-4-name\").text(\"";
    INDEX_HTML += channelNames[3];
    INDEX_HTML +="\"); $(\"#chn-1-timer\").text(\"";
    INDEX_HTML += (channelTimers[0]>60)? float(channelTimers[0]/60) : channelTimers[0];
    INDEX_HTML += (channelTimers[0]>60)? " min" : " sec";
    INDEX_HTML +="\"); $(\"#chn-2-timer\").text(\"";
    INDEX_HTML += (channelTimers[1]>60)? float(channelTimers[1]/60) : channelTimers[1];
    INDEX_HTML += (channelTimers[1]>60)? " min" : " sec";
    INDEX_HTML +="\"); $(\"#chn-3-timer\").text(\"";
    INDEX_HTML += (channelTimers[2]>60)? float(channelTimers[2]/60) : channelTimers[2];
    INDEX_HTML += (channelTimers[2]>60)? " min" : " sec";
    INDEX_HTML +="\"); $(\"#chn-4-timer\").text(\"";
    INDEX_HTML += (channelTimers[3]>60)? float(channelTimers[3]/60) : channelTimers[3];
    INDEX_HTML += (channelTimers[3]>60)? " min" : " sec";
    INDEX_HTML +="\"); $(\"#chn-1-time\").text(\"";
    // INDEX_HTML += myVariable;
    INDEX_HTML +="12:00\"); $(\"#chn-2-time\").text(\"";
    // INDEX_HTML += myVariable;
    INDEX_HTML +="12:00\"); $(\"#chn-3-time\").text(\"";
    // INDEX_HTML += myVariable;
    INDEX_HTML +="12:00\"); $(\"#chn-4-time\").text(\"";
    // INDEX_HTML += myVariable;
...
void startTimer(int channel){
  if(channelTime[i-1]==""){
    int timerSec=channelTimers[i-1]mod60;
    int timerMin=(channelTimers[i-1]/60)mod60;
    int timerHours=(channelTimers[i-1]/60)/60;

    channelTime[i-1]=(timeClient.getHours+timerHours)+":"+(timeClient.getMinutes+timerMin)+":"+(timeClient.getSeconds+timerSec);//getTime+channelTimers
  }
}
